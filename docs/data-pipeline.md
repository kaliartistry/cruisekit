...
## Legacy rich-catalog ship-code normalization

The pre-canonical mobile `sailing_catalog.json` is not generated by the active
web bundle pipeline. Some historical rows stored a two-letter provider ship
code in `shipName`. The reviewed code-to-name reference lives at
`data/reference/ship-code-names.json` and can be applied to an explicitly
supplied catalog without editing the raw archived captures:

```bash
pnpm data:normalize:ship-codes -- \
  --input /path/to/sailing_catalog.json \
  --output /path/to/reviewed/sailing_catalog.json

pnpm data:normalize:ship-codes -- \
  --input /path/to/reviewed/sailing_catalog.json \
  --check
```

The normalizer preserves `shipCode`, changes only a blank or bare-code
`shipName` with an authoritative mapping, and reports unresolved bare codes.
It has no default input or output path so this repository does not silently
write into a neighboring mobile checkout. Shipping a corrected bundled catalog
